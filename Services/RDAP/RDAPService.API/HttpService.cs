...
namespace RDAPService.API
{
    public abstract class HttpService
    {
        ...
        protected async Task<T> Get<T>(string endpoint, string token = null)
        {
            ...
            {
                _logger.LogError($"Microservice authorization exception while executing request " +
                    $"{HttpMethod.Get} {endpoint} - Exception {authorizationException.Message} " +
                    $"- Inner exception {authorizationException.InnerException?.Message}");
                throw;
            }
            catch (Exception ex)
            {
                _logger.LogError($"Error while executing request {HttpMethod.Get} {_client.BaseAddress + endpoint} " +
        ...
        private async Task<HttpResponseMessage> Execute(HttpRequestMessage request, string token = null)
        {
            ...
            {
                _logger.LogError($"Request to the microservice unauthorized with reason phrase " +
